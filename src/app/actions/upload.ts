...
import { generatePostHero } from "@/lib/ai-illustrator";
...
export async function generateAndSaveHero(slug: string): Promise<{
  url?: string;
  error?: string;
}> {
  try {
    ...
    const url = await generatePostHero(post.content, slug);
